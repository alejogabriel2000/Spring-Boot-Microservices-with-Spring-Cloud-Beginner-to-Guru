...
public class OrdenCervezaStateMachineConfig extends StateMachineConfigurerAdapter<OrdenEstadoCervezaEnum, OrdenEventoCervezaEnum> {
   ...
   public void configure(StateMachineStateConfigurer<OrdenEstadoCervezaEnum, OrdenEventoCervezaEnum> estados) throws Exception {
      ...
      estados.withStates()
         ...
         .end(OrdenEstadoCervezaEnum.CANCELADO)
   ...
   public void configure(StateMachineTransitionConfigurer<OrdenEstadoCervezaEnum, OrdenEventoCervezaEnum> transiciones) throws Exception {
      transiciones.withExternal()
         ...
         .and().withExternal()
             .source(OrdenEstadoCervezaEnum.VALIDACION_PENDIENTE).target(OrdenEstadoCervezaEnum.CANCELADO)
         ...
         .and().withExternal()
             .source(OrdenEstadoCervezaEnum.VALIDADO).target(OrdenEstadoCervezaEnum.CANCELADO)
             .event(OrdenEventoCervezaEnum.CANCELAR_ORDEN)
         .and().withExternal()
         ...
         .and().withExternal()
             ...
             .action(asignarFallaAction)
         .and().withExternal()
             .source(OrdenEstadoCervezaEnum.ASIGNADO_PENDIENTE).target(OrdenEstadoCervezaEnum.CANCELADO)
             .event(OrdenEventoCervezaEnum.CANCELAR_ORDEN)
         .and().withExternal()
