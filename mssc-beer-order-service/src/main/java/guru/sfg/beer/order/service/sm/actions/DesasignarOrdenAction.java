...
@Component
public class DesasignarOrdenAction implements Action<OrdenEstadoCervezaEnum, OrdenEventoCervezaEnum> {
   ...
   public void execute(StateContext<OrdenEstadoCervezaEnum, OrdenEventoCervezaEnum> stateContext) {
      ...
      Optional<BeerOrder> cervezaOrdenOptional = beerOrderRepository.findById(UUID.fromString(Objects.requireNonNull(cervezaOrdenId)));
      cervezaOrdenOptional.ifPresentOrElse(cervezaOrden -> {
         jmsTemplate.convertAndSend(JmsConfig.DESASIGNAR_ORDEN_QUEUE,
                                    DesasignarOrdenRequest.builder()
                                                          .cervezaOrden(beerOrderMapper.beerOrderToDto(cervezaOrden))
                                                          .build());
         log.debug("Enviada desubicacion request para la orden con ID: " + cervezaOrdenId);
