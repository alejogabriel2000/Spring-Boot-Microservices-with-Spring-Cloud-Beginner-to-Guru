...
public class CervezaControllerV2 {
   ...
   @GetMapping({ "/{cervezaId}" }) public ResponseEntity<CervezaDTOV2> getCerveza(@PathVariable("cervezaId") UUID cervezaId) {

      return new ResponseEntity<>(cervezaServiceV2.getCervezaById(cervezaId), HttpStatus.OK);
   }

   @PostMapping public ResponseEntity handlePost(@RequestBody CervezaDTOV2 cervezaDTO) {
...
/*    @PostMapping
   ...
   @PutMapping({ "/{cervezaId}" }) public ResponseEntity handleUpdate(@PathVariable("cervezaId") UUID cervezaId, @RequestBody CervezaDTOV2 cervezaDTO) {
