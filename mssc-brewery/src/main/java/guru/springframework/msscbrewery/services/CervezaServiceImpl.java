...
@Slf4j @Service public class CervezaServiceImpl implements CervezaService {
   @Override public CervezaDTO getCervezaById(UUID beerId) {
      return CervezaDTO.builder().id(UUID.randomUUID()).nombreCerveza("Patagonia").estiloCerveza("Patagonia Pale Ale").build();
   }

   @Override public CervezaDTO grabarNuevaCerveza(CervezaDTO cervezaDTO) {
      return CervezaDTO.builder().id(UUID.randomUUID()).build();
   }
